...
class VdoCipher:
    ...
    def __init__(self, arg) -> None:
        ...
        self.idx = eval(
            base64.urlsafe_b64decode(
                (eval(
                    base64.urlsafe_b64decode(
                        self.args.auth.encode("utf-8")).decode("utf-8"))
                 ["playbackInfo"]).encode("utf-8")).decode("utf-8"))["videoId"]
    ...
    def header(self):
        headers = {
            ...
            eval(
                base64.urlsafe_b64decode(
                    self.args.auth.encode("utf-8")).decode("utf-8"))["href"],
            "origin":
            self.ref,
            "referer":
            self.ref + "/",
    ...
    def check(self):
        ...
        change = base64.b64encode(str(
            json.dumps(payloads_token)).encode()).decode()
        self.json_payloads["token"] = change
        challenge = self.json_payloads
        r = requests.post(self.args.license_url,
                          json=challenge,
